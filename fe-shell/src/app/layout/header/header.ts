  ...
  template: `
    <header class="navbar bg-base-100 shadow-sm px-4">
        ...
        <div class="dropdown lg:hidden">
          ...
          <ul tabindex="0" class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-56 p-2 shadow">
            ...
            @if (auth.isAuthenticated()) {
              ...
              <li>
                <a routerLink="/login" routerLinkActive="menu-active">Login</a>
              </li>
            }
          </ul>
        </div>
      ...
      <div class="navbar-end gap-2">
        <label class="swap swap-rotate btn btn-ghost btn-circle" aria-label="Dark Mode umschalten">
          <input
            type="checkbox"
        ...
        @if (auth.isAuthenticated()) {
          <div class="dropdown dropdown-end">
            <button type="button" tabindex="0" class="btn btn-ghost" aria-haspopup="true">
              {{ auth.currentUser()?.name ?? auth.currentUser()?.email }}
